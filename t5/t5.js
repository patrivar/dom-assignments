...
const map = L.map('map').setView([60.224, 24.759], 11);
...
function success(pos) {
  const { coords } = pos;
  omaPaikka = [coords.longitude, coords.latitude];
  ...
  for (let restaurant of restaurants) {
    ...
    const etaisyysSolu = document.createElement('td');
    etaisyysSolu.innerText = distance(omaPaikka, restaurant.location.coordinates).toFixed(2);

    rivi.append(nimiSolu, osoiteSolu, etaisyysSolu);
    target.appendChild(rivi);

    const coords = [...restaurant.location.coordinates].reverse();
